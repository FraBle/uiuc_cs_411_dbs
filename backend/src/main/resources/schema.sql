...
(50) NOT NULL,
  ...
  `Position` ENUM
('C','C-F','F','F-C','F-G','G','G-F') NOT NULL,
  `Height` VARCHAR
(10) NOT NULL,
  `Weight` INT NOT NULL,
  `College` INT
...
DROP VIEW FranchiseWinsBySeason
IF EXISTS;
DROP VIEW FranchiseWins
IF EXISTS;
DROP TABLE Games
IF EXISTS;
CREATE TABLE `Games`
...
DROP VIEW `PlayerAllStats`
IF EXISTS;
DROP VIEW `PlayerSeasonStats`
IF EXISTS;
DROP VIEW `PLAYERFRANCHISESTATS`
IF EXISTS;
...
(11) NOT NULL,
  `Franchise` int
(11) NOT NULL,
  `MinutesPlayed` tinyint
(4) NOT NULL,
  `FieldGoalsMade` tinyint
(4) NOT NULL,
  `FieldGoalsAttempted` tinyint
(4) NOT NULL,
  `ThreePointersMade` tinyint
(4) NOT NULL,
  `ThreePointersAttempted` tinyint
(4) NOT NULL,
  `FreeThrowsMade` tinyint
(4) NOT NULL,
  `FreeThrowsAttempted` tinyint
(4) NOT NULL,
  `OffensiveRebounds` tinyint
(4) NOT NULL,
  `DefensiveRebounds` tinyint
(4) NOT NULL,
  `Points` tinyint
(4) NOT NULL,
  `Assists` tinyint
...
SELECT FranchiseId, CONCAT(Franchise.City, ' ', Franchise.Nickname) as FranchiseName, Season, COUNT(*) AS VICTORIES
FROM
  (            SELECT HomeFranchise AS FranchiseId, Season
    FROM Games
    WHERE HomePoints > AwayPoints
  UNION ALL
    SELECT VisitorFranchise AS FranchiseId, Season
    FROM Games
    WHERE AwayPoints > HomePoints) tmp
  JOIN Franchise ON (Franchise.ID=FranchiseId)
GROUP BY FranchiseId, Season;

CREATE VIEW `FranchiseWins` AS
SELECT FranchiseId, FranchiseName, SUM(VICTORIES) AS VICTORIES
FROM FranchiseWinsBySeason
GROUP BY FranchiseId
ORDER BY VICTORIES DESC;

DROP INDEX IDXURLSHORT
IF EXISTS;
DROP TABLE ShortURL
IF EXISTS;
