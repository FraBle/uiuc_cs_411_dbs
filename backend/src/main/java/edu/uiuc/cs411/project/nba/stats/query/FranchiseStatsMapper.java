...
public interface FranchiseStatsMapper {
  ...
  List<FranchiseStats> topFranchiseStatsBySeason(@Param("season") Integer season, @Param("sortType") String sortType,
      @Param("topN") Integer n);

  @Select("SELECT * FROM FranchiseWins WHERE FranchiseId=${franchiseId}")
  FranchiseWins getFranchiseWins(int franchiseId);

  @Select("SELECT * FROM FranchiseWinsBySeason WHERE FranchiseId=${franchiseId} AND Season=${season}")
  ...
  @Select("SELECT * FROM FranchiseWinsBySeason WHERE SEASON=${season} ORDER BY VICTORIES DESC, FranchiseId LIMIT ${topN}")
